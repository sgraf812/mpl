...
elab_rules : term
  | `(⦃$P⦄ $x:term ⦃$Q⦄) => do
    ...
    let P ← elabTerm (← `(sprop($P))) (mkApp (mkConst ``PreCond) ps)
    let Q ← elabTerm (← `(sprop($Q))) (mkApp2 (mkConst ``PostCond) α ps)
...
theorem triple_conseq {α} (x : m α) {P P' : PreCond ps} {Q Q' : PostCond α ps}
  ...
  triple x P Q := by
    apply SProp.entails.trans hp
    apply SProp.entails.trans h
...
theorem triple_pure [Monad m] [MonadMorphism m (PredTrans ps) wp] {α} {Q : PostCond α ps} (a : α) (himp : P.entails (Q.1 a)) :
  triple (pure (f:=m) a) P Q := by apply SProp.entails.trans himp (by simp only [pure_pure, PredTrans.pure_apply, SProp.entails.refl])
