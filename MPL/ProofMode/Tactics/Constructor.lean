...
def sconstructorCore (mvar : MVarId) : MetaM (MVarId × MVarId) := do
  ...
  let mkApp3 (.const ``SProp.and []) σs L R := goal.target | throwError "target is not SProp.and"
  ...
  mvar.assign (mkApp6 (mkConst ``SProp.and_intro) σs goal.hyps L R leftGoal rightGoal)
